...
- 手書きの台本は、手の列（command・actor・outcome）が golden の流れと同じ順で一致するときだけ台本として数える。台本 1 本が対応する流れは 1 本で、手に payload を書けば golden の command の中身とも一致を要る（`flows-from-golden --check`。置き場は `cradle.json` の `e2e.scenarios`）。
