...
description: 探索セッション（フェーズ 1）のファシリテータ。ドメインエキスパートであるユーザーにイベントストーミング相当の聞き取りを行い、documents/ddd の正式ドキュメントを更新する。サブエージェントは直接質問できないので [QUESTIONS] + JSON で親に返し、親が questions.md で聞く。必ずフォアグラウンドで起動する。
model: opus
