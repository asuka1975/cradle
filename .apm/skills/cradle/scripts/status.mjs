...
if (!existsSync(R(cfg.backend.dir))) phase("バックエンド", "未着手", [], "モデルの画面確認が済んでから着手（順序）");
  ...
  const ports = gen.flatMap(g => walk(g, { ext: [".kt"] })).flatMap(f => { const m = readFileSync(f, "utf8").match(/^ \* Lean: Port `[^`]+`[\s\S]*?^interface (\w+)/m); return m ? [m[1]] : []; });
  ...
  const portFact = ports.length ? `Port ${ports.length} 件（Adapter ${adapters.length}）` : null;
  phase("バックエンド", gen.length ? "着手済" : "生成前", [`生成物 ${gen.length ? "あり" : "なし"}`, `契約テスト 配線 ${wired}/${abstractNames.length}`, portFact, `手書き ${impls} ファイル`].filter(Boolean),
    !gen.length ? `${cfg.backend.regenerate} で生成` : wired < abstractNames.length ? `未配線の契約テスト ${abstractNames.length - wired} 件を具象クラスで繋ぐ`
      : adapters.length < ports.length ? `Adapter の無い Port ${ports.filter(p => !adapters.includes(p)).join(", ")} を infrastructure/<port>/ に書く` : "cradle regen-impact で追随漏れを確かめる");
}
