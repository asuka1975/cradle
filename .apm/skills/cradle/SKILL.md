...
すべて `cradle <command>`（このスキルの `scripts/cradle.mjs`。呼び方は cradle-core 規則のとおり、プロジェクトルートで実行）。
...
- どの道具も `CRADLE_PROJECT_DIR` でルートを指定できる（hook は Claude Code の `CLAUDE_PROJECT_DIR`、無ければ stdin の `cwd` から `cradle.json` を持つ祖先を探す）。
