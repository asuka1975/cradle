...
成果物は `documents/infra-design/`（README + 01_model-to-infra + 02_local + 03_production + 04_operations）。実装（`infra/`）はあとで、設計に従う。
...
4. 決定を INFRA-D-xxx として根拠（モデルの定理名・HS）付きで表に書く。裏付けの無い仮定は INFRA-A-xxx（サイジングにだけ使う）。ユーザーの判断が要るものは INFRA-Q-xxx として問いの形で残し、暫定を書く。
...
- 設計に日付付きの進捗チェックリストを残さない。状態は `cradle status` と git が持つ。
