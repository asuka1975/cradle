...
`/cradle-init` が敷く最小ドメイン（メモを書く・閉じる・一覧）の形がそのまま規約の実例。例はその語彙で書く。
...
観測モデル（`Application/RepositoryState.lean`）は Repository の論理状態。表現はリスト（並びが業務の情報）、不変条件は per-Root の `valid`。
点更新は `update`（同一性を変えない f だけ）、追加は `add`（末尾）。消す操作は無い。ID の泉は `Fountain σ α`（値型と生成器状態は抽象、具体化は Runtime）。
...
def validate (_actor : ActorContext UserId) (c : Command) (_before : State …) : Except DomainError Unit :=
  if c.title.valid then .ok () else .error .emptyTitle           -- 始まる前の拒否だけ
def act (actor) (fountain : Fountain G NoteId) (c) (before) (_ : Unit) : State … := …   -- Entity のふるまいの適用一発
def execute (actor) (fountain) (c) (before) := (validate actor c before).map (act actor fountain c before)
```

- validate の戻り値: 既存集約を変える UseCase = 解決の成果物（集約ルート）、新規追加 = `Unit`、参照系 = `Unit`。
- 契約定理: エラー枝 1 本 = 定理 1 本（`execute_<error>`）、成功（`execute_ok`）、追加 / 更新の状態の等式（`act_appends` 等）、泉の消費、フレーム（触らないコレクション）、`act_preserves_valid`。
  境界の可到達性が使う `execute_ok_shape`（成功なら結果は act の形）も置く。
- 名義（`ActorContext`）・時計・泉はポート位置（先頭）。ペイロードに混ぜない。
...
読み取りの保証を境界の言葉に移す。整合性の仮定は `h : s.Reachable` だけから取る（露出仮定を残さない）。check に無い事実が要るなら valid に条項を足す（各 UseCase の保存義務になり Reachable が運ぶ）。
