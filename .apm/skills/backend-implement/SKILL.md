...
3. **実装**（規約は backend-kotlin / backend-design）:
   - UseCase: 固定形（validate が解決、execute が validate を呼んで作用、境界は execute）。内部入力（`Observation`）の UseCase も同じ固定形で、名義を受けない。Port を使う配送は送る印を保存してから Port を呼ぶ（backend-kotlin 規則）。
