...
Cradle の骨格（`.apm/skills/cradle-init/assets/lean`、名前空間 `Sprout`）を `build/scaffold/lean` に写し、Gradle plugin `dev.lean2kotlin` で抽出 → 生成 → コンパイル → 生成された抽象契約テスト 7 本を InMemory 実装（`src/main`）と具象サブクラス（`src/test`）で通す。配線は消費側と同じ形（生成器はこのリポジトリの `lean2kotlin/` を `includeBuild`。別の checkout は `LEAN2KOTLIN_HOME`）。
