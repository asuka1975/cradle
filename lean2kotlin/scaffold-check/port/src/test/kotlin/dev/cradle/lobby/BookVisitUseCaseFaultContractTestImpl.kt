...
class BookVisitUseCaseFaultContractTestImpl : BookVisitUseCaseFaultContractTest() {
	...
	/** 保存の途中で技術的障害が起きる仕掛け: 渡された Repository の add だけを例外で止める（観測はその Repository で行う。ディレクトリは呼ばれた後）。 */
	override fun faultedSavingFailed(visitRepository: VisitRepository, organizationDirectory: OrganizationDirectory, visitIdGenerator: VisitIdGenerator, actor: ActorContext): BookVisitUseCase =
