// GENERATED by lean2kotlin — DO NOT EDIT
// 再生成で全上書きされます。実装はこのファイルではなく別ファイルに書いてください。

package dev.cradle.lobby.application.usecase.bookvisitusecase

import dev.cradle.lobby.DomainResult
import dev.cradle.lobby.domain.DomainError

/**
 * Lean: モジュール `BookVisitUseCase` の interface 面(署名は Lean の def の写し)。
 * アプリケーション層の UseCase(固定形 validate / execute)。
 * 取得(読み取りストア)・乱択の鍵・調達ポート(時計・主体・外部能力の観測)・precise な入力
 * (lookup 結果・採番列)は**実装の配線**(注入ポート)— 署名には現れない
 * (証明装置を本番界面に写さない)。効果はリポジトリ経由で観測する。
 * 実装は AI が別ファイルに書く。default 禁止。
 */
interface BookVisitUseCase {
	/** execute は validate を通してから観測を適用する。 */
	fun execute(c: BookVisitCommand): DomainResult<DomainError, Unit>
	/**
	 * 始まる前の拒否: 来訪者の名前が空なら受け付けない、受付中の来訪があれば受け付けない。
	 * 本番では execute の内部第一段として呼ばれる(実装は「execute は
	 * validate を呼ぶ」形に一本化する — 分岐条件を再実装しない)。
	 * 公開メンバとしては**テストシーム**(定理→テスト 1:1 導出・読み取り専用の検査)。
	 */
	fun validate(c: BookVisitCommand): DomainResult<DomainError, BookVisitVacant>
}
